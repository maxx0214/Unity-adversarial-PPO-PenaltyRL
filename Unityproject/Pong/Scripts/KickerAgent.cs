...
public class KickerAgent : Agent
{
    ...
    // 최대 거리 보상 강도 설정 (1차 피드백 후 추가) (2차 피드백 후 수치 변경 5 -> 1)
    private const float MAX_DIST_REWARD = 0.0001f;

    // Kicker가 공에 대해 가질 수 있는 최대 거리 (1차 피드백 후 추가)
    private const float MAX_DISTANCE = 25f;

    // 이전 PongAgent와 동일한 변수들을 유지합니다.
    ...
    private const int Z_Stay = 0;
    private const int Z_UP = 1;
    private const int Z_DOWN = 2;

    // X-Movement Actions (Branch 1) - KickerAgent를 위해 추가
    private const int X_Stay = 0;
    ...
    private const float X_Clamp = 10.0f; // 3차 피드백 후 X축 제한 변경 (공까지만)
    ...
    public override void OnActionReceived(ActionBuffers actionBuffers)
    {
        ...
        switch (zAction)
        {
            ...
            case Z_DOWN:
                ...
                break;
                // case Z_Stay: (Do nothing)
        }

        // 2. X축 움직임 가로방향 (LEFT/RIGHT) - KickerAgent의 핵심 추가 동작
        switch (xAction)
        {
            case X_RIGHT:
            ...
            case X_LEFT:
                ...
                break;
                // case X_Stay: (Do nothing)
        }

        // 3. 경계 제한) 3차 피드백 후 kicker의 x축 추가 제한 (공까지만 이동 가능) (X_Clamp 값 : 12.5 -> 10.0) 
        ...
        // 4. 거리 기반 보상 부여 (공에 가까워질수록 보상 증가)
        // 1. Kicker와 공 사이의 거리 계산
        float distanceToBall = Vector3.Distance(transform.localPosition, Ball.transform.localPosition);

        // 2. 거리를 정규화 (0 ~ 1 사이 값)
        float normalizedDistance = Mathf.Clamp(distanceToBall / MAX_DISTANCE, 0f, 1f);

        // 3. 거리가 가까울수록 보상을 크게 부여 (1.0 - normalizedDistance)
